...
O_Node* insertion_sort(O_Node* head_ptr);
...
int main() {
    ...
    first:
        ...
        case 4:
            ...
            break;
        case 5:
            printOOTDList(insertion_sort(&head2), printOOTDInfo); //오름차순 정렬 O
            break;
        case 6:
            break;
        case 7:
...
O_Node* insertion_sort(O_Node* head_ptr) {
    ...
    while (p->next != NULL) {
        if (p->ootd.date > p->next->ootd.date)
        {
            ...
            temp->next = head_ptr; // temp의 next는 가장 첫 자리를 가리킵니다.
            head_ptr = p = temp; // 그리고 p와 head_ptr 모두 temp로 초기화 함으로 써 모두 가장 첫자리로 가게 됩니다.       
